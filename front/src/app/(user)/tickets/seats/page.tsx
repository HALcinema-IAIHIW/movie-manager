...
function SeatSelectionContent() {
    ...
    const handleNext = async () => {
        ...
        try {
            ...
            if (!Array.isArray(registeredSeats) || registeredSeats.length === 0) {
                console.warn("registeredSeats が空または不正です", {
                    screenId,
                    type: typeof registeredSeats,
                    length: Array.isArray(registeredSeats) ? registeredSeats.length : null,
                });
            }
            ...
            const seatsToBook = selectedSeats.map(uiSeatId => {
                ...
                if (!foundSeat) {
                    console.error("座席データが見つかりません", {
                        uiSeatId,
                        row,
                        column,
                        screenId,
                        registeredSeatsCount: Array.isArray(registeredSeats) ? registeredSeats.length : null,
                    });
