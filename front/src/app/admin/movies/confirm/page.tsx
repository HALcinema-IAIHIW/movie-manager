...
export default function MovieConfirm() {
  ...
  const handleSubmit = async () => {
    ...
    console.log("フォームデータ確認:", {
      ...
      hasDates: !!(formData.startDate && formData.endDate)
    });
    try {
      const token = localStorage.getItem("adminToken")
      ...
      console.log("Movie Created ID:", newMovieId)

      // 2. 上映期間情報の登録 (/admin/screening-periods)
      if (formData.startDate && formData.endDate) {
        ...
        console.log("送信データ:", periodPayload);

        const periodRes = await fetch("http://localhost:8080/periods", {
          method: "POST",
          headers: {
            "Authorization": `Bearer ${token}`,
            "Content-Type": "application/json"
        ...
        if (!periodRes.ok) {
          console.error(`上映期間の登録に失敗しました (Status: ${periodRes.status})`);
          ...
          console.log("上映期間登録成功:", periodPayload)
        }
      }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-accent/20">
                    ...
                    {formData.endDate && (
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-sm text-text-secondary font-shippori block">公開終了日</span>
                            <span className="text-text-primary font-jp">{formatDate(formData.endDate)}</span>
                          </div>
                        </div>
                    )}
