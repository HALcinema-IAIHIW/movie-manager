...
export default function AdminTicketType() {
    ...
    if (isLoading) {
        return <div className="min-h-screen pt-24 flex items-center justify-center text-text-primary bg-darkest">Loading...</div>
    }

    if (error) {
        return <div className="min-h-screen pt-24 flex items-center justify-center text-red-500 bg-darkest">Error: {error}</div>
    }

    if (!movieInfo) {
        return <div className="min-h-screen pt-24 flex items-center justify-center text-text-primary bg-darkest">映画情報がありません。</div>
    }
    ...
    return (
      <div className="min-h-screen bg-darkest text-text-primary">
          {/* ... JSX部分は変更なし ... */}
          <section>
              <div className="container-luxury flex items-baseline gap-4 pt-8">
                  <h1 className="text-4xl font-bold text-text-primary font-en">券種選択</h1>
              </div>
          </section>
          ...
          <section className="py-8">
              <div className="container-luxury">
                  {/* 映画情報 */}
                  {/*<h1 className="mb-4 text-text-muted">ここはAdminの券種選択です</h1>*/}
                  <div className="bg-dark-lighter rounded-lg p-6 mb-8">
                      <div className="flex items-center gap-6">
                          ...
                          <div className="flex-1">
                              <h2 className="text-4xl font-bold text-gold mb-2 font-jp">{movieInfo.title}</h2>
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-text-secondary">
                                  <div className="flex items-center gap-2">
                                      <Calendar size={16} className="text-gold" />
                                      <span className="font-jp text-2xl">{movieInfo.date}</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                      <Clock size={16} className="text-gold" />
                                      <span className="font-jp text-2xl">{movieInfo.time}</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                      <MapPin size={16} className="text-gold" />
                                      <span className="font-jp text-2xl">{movieInfo.screen}</span>
                                  </div>
                              </div>
                          </div>
                      </div>
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                          ...
                          <div className="bg-dark-lighter rounded-lg p-8">
                              ...
                              <div className="space-y-6">
                                  {seatTickets.map((seatTicket) => (
                                    <div key={seatTicket.seatIdStr} className="bg-darkest rounded-lg p-6">
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-2xl font-medium text-text-primary font-jp">
                                        ...
                                        <div>
                                            <label className="block text-xl font-medium text-text-secondary mb-2 font-jp">券種を選択</label>
                                            <select
                                              value={seatTicket.ticketType.id}
                                              onChange={(e) => handleTicketTypeChange(seatTicket.seatIdStr, e.target.value)}
                                              className="w-full px-4 py-3 bg-dark-lighter rounded-lg
                             text-text-primary focus:outline-none transition-colors font-jp"
                                            >
                              ...
                              {/* 券種説明 */}
                              <div className="mt-8 p-6 bg-darkest rounded-lg">
                                  <h4 className="text-lg font-medium text-text-primary mb-4 font-jp">券種について</h4>
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-md text-text-secondary">
                                      <div>
                      ...
                      <div className="xl:col-span-1">
                          <div className="bg-dark-lighter rounded-lg p-6 sticky top-32">
                              <h3 className="text-2xl font-bold text-text-primary mb-4 flex items-center gap-2 font-jp">
                              ...
                              <div className="space-y-3 mb-6">
                                  {seatTickets.map((seatTicket) => (
                                    <div key={seatTicket.seatIdStr} className="flex items-center justify-between p-3 bg-darkest rounded-lg">
                                        <div>
                                            <p className="text-text-primary font-medium font-en">{seatTicket.seatIdStr}</p>
                                            <p className="text-md text-text-secondary font-jp">{seatTicket.ticketType.name}</p>
                                        </div>
                                        <p className="text-xl text-gold font-medium font-playfair">
                                            ¥{seatTicket.ticketType.price.toLocaleString()}
                                        </p>
                                    </div>
                                  ))}
                              ...
                              <div className="mt-4 mb-6">
                                  <div className="flex items-center justify-between">
                                      <span className="text-2xl font-medium text-text-primary font-jp">合計金額</span>
                                      <span className="text-4xl font-bold text-gold font-playfair">¥{totalPrice.toLocaleString()}</span>
                                  </div>
                              </div>
                       ...
                       bg-gold text-darkest hover:bg-gold/90
                       rounded-lg font-medium transition-colors duration-200 font-jp"
                                  >
                                      お支払いへ進む
                                      <ArrowRight size={18} />
                                  </button>
                                  ...
                                  <button
                                    onClick={handleBack}
                                    className="w-full flex items-center justify-center gap-2 py-3 px-4
                       text-gold hover:bg-darkest rounded-lg transition-colors duration-200 font-jp"
                                  >
                                      <ArrowLeft size={18} />
                                      座席選択に戻る
                                  </button>
